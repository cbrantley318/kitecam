...
void msgReceiveCb(const esp_now_recv_info *macAddr, const uint8_t *data, int dataLen) {
...
void msgReceiveCbByMAC(const esp_now_recv_info* macAddr, const uint8_t *data, int dataLen) {
  // printMacAddress(macAddr); 
  // std::lock_guard<std::mutex> lck(recv_cb_mtx);
  uint32_t id = getReceiverId((const uint8_t*)macAddr);
